...
def test_history_analyzer_initialization():
    ...
    assert analyzer.history_file == get_config_value("history_file", os.path.expanduser("~/.bash_history"))
...
def test_read_and_analyze_history():
    ...
    try:
        ...
        assert common_commands.get("git") in (2, 3)  # git appears 2-3 times depending on implementation
    finally:
        # Clean up
